...
/* This function is used to build a tree of duplicates by adding a new__ node to
...
__eb_insert_dup(struct eb_node *sub, struct eb_node *new__)
{
	struct eb_node *head = sub;
	
	eb_troot_t *new_left = eb_dotag(&new__->branches, EB_LEFT);
	eb_troot_t *new_rght = eb_dotag(&new__->branches, EB_RGHT);
	eb_troot_t *new_leaf = eb_dotag(&new__->branches, EB_LEAF);
	...
	if (head->bit < -1) {
		...
		sub = container_of(eb_untag(head->branches.b[EB_RGHT], EB_LEAF),
				   struct eb_node, branches);
		head->branches.b[EB_RGHT] = eb_dotag(&new__->branches, EB_NODE);

		new__->node_p = sub->leaf_p;
		new__->leaf_p = new_rght;
		sub->leaf_p = new_left;
		new__->branches.b[EB_LEFT] = eb_dotag(&sub->branches, EB_LEAF);
		new__->branches.b[EB_RGHT] = new_leaf;
		return new__;
	} else {
		int side;
		 ...
		 */
		new__->bit = sub->bit - 1; /* install at the lowest level */
		side = eb_gettag(sub->node_p);
		head = container_of(eb_untag(sub->node_p, side), struct eb_node, branches);
		head->branches.b[side] = eb_dotag(&new__->branches, EB_NODE);
					
		new__->node_p = sub->node_p;
		new__->leaf_p = new_rght;
		sub->node_p = new_left;
		new__->branches.b[EB_LEFT] = eb_dotag(&sub->branches, EB_NODE);
		new__->branches.b[EB_RGHT] = new_leaf;
		return new__;
	}
}
...
static forceinline void __eb_delete(struct eb_node *node)
{
	...
	/* We must now update the new__ node's parent... */
	gpside = eb_gettag(parent->node_p);
	gparent = eb_untag(parent->node_p, gpside);
				  ...
				  int ignore, int len)
{
	for (ignore >>= 3, a += ignore, b += ignore, ignore <<= 3;
		...
		if (c) {
			/* OK now we know that old and new__ differ at byte <ptr> and that <c> holds
...
REGPRM1 struct eb_node *eb_insert_dup(struct eb_node *sub, struct eb_node *new__);
