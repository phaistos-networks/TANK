...
auto get_outgoing_queue() {
        auto res =  outgoingQueuesPool.size() ? outgoingQueuesPool.Pop() : new outgoing_queue();
...
Switch::shared_refptr<topic_partition> init_local_partition(const uint16_t idx, topic *, const partition_config &);
...
static inline auto normalized_max_wait(const uint64_t v) noexcept {
