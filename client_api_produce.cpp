...
bool TankClient::process_produce(connection *const c, const uint8_t *const content, const size_t len) {
        ...
        while (p < end) {
                ...
                if (err == 0xff || err == 0xfe) {
                        ...
                        } while ((part_it = next) != &br_req->partitions_list && (req_part = switch_list_entry(request_partition_ctx, partitions_list_ll, part_it))->topic == topic);
                ...
                for (;;) {
                        ...
                        if (trace) {
                                SLog("Considering ", req_part->topic, "/", req_part->partition, " ", err, "\n");
                        }
                        ...
                        part_it = next;
                        if (part_it == &br_req->partitions_list) {
                                break;
                        } else {
