...
REGPRM2 struct eb32_node *eb32_insert(struct eb_root *root, struct eb32_node *new__);
REGPRM2 struct eb32_node *eb32i_insert(struct eb_root *root, struct eb32_node *new__);
...
/* Insert eb32_node <new__> into subtree starting at node root <root>.
 * Only new__->key needs be set with the key. The eb32_node is returned.
 * If root->b[EB_RGHT]==1, the tree may only contain unique keys.
 */
static forceinline struct eb32_node *
__eb32_insert(struct eb_root *root, struct eb32_node *new__) {
	...
	new__->node.bit = flsnz(new__->key ^ old->key) - EB_NODE_BITS;
...
/* Insert eb32_node <new__> into subtree starting at node root <root>, using
 * signed keys. Only new__->key needs be set with the key. The eb32_node
 * is returned. If root->b[EB_RGHT]==1, the tree may only contain unique keys.
 */
static forceinline struct eb32_node *
__eb32i_insert(struct eb_root *root, struct eb32_node *new__) {
	struct eb32_node *old;
	unsigned int side;
	eb_troot_t *troot, **up_ptr;
