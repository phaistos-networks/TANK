...
REGPRM2 struct ebpt_node *ebis_insert(struct eb_root *root, struct ebpt_node *new__);
...
/* Insert ebpt_node <new__> into subtree starting at node root <root>. Only
 * new__->key needs be set with the zero-terminated string key. The ebpt_node is
 * returned. If root->b[EB_RGHT]==1, the tree may only contain unique keys. The
 * caller is responsible for properly terminating the key with a zero.
 */
static forceinline struct ebpt_node *
__ebis_insert(struct eb_root *root, struct ebpt_node *new__)
{
	...
	while (1) {
		if (unlikely(eb_gettag(troot) == EB_LEAF)) {
			 ...
			 */
			if (bit >= 0)
				bit = string_equal_bits(new__->key, old->key, bit);
			...
			if (bit < 0) {
				...
				/* new__ arbitrarily goes to the right and tops the dup tree */
		...
		if (unlikely(bit < 0)) {
			 ...
			 * <new__> before the node <old>, and set ->bit to designate the lowest
			 * bit position in <new__> which applies to ->branches.b[]. In the later
		...
		side = (((unsigned char *)new__->key)[old_node_bit >> 3] >> (~old_node_bit & 7)) & 1;
		troot = root->b[side];
	}
