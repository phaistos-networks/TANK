...
REGPRM3 struct ebmb_node *ebmb_insert(struct eb_root *root, struct ebmb_node *new__, unsigned int len);
...
REGPRM3 struct ebmb_node *ebmb_insert_prefix(struct eb_root *root, struct ebmb_node *new__, unsigned int len);
...
/* Insert ebmb_node <new__> into subtree starting at node root <root>.
 * Only new__->key needs be set with the key. The ebmb_node is returned.
...
__ebmb_insert(struct eb_root *root, struct ebmb_node *new__, unsigned int len)
{
	...
	while (1) {
		...
		check_bit_and_break:
			bit = equal_bits(new__->key, old->key, bit, len << 3);
			break;
		}
		...
		bit = equal_bits(new__->key, old->key, bit, old_node_bit);
		if (unlikely(bit < old_node_bit)) {
			/* The tree did not contain the key, so we insert <new__> before the
			 * node <old>, and set ->bit to designate the lowest bit position in
			 * <new__> which applies to ->branches.b[].
			 */
			new__->node.node_p = old->node.node_p;
			up_ptr = &old->node.node_p;
			break;
		...
		side = old_node_bit & 7;
		side ^= 7;
		side = (new__->key[old_node_bit >> 3] >> side) & 1;
		troot = root->b[side];
	}
	...
	if (((unsigned)bit >> 3) < len)
		diff = cmp_bits(new__->key, old->key, bit);

	if (diff == 0) {
		new__->node.bit = -1; /* mark as new__ dup tree, just in case */
	...
	if (diff >= 0) {
		new__->node.branches.b[EB_LEFT] = troot;
		...
		*up_ptr = new_left;
		...
		new__->node.branches.b[EB_RGHT] = troot;
...
/* Insert ebmb_node <new__> into a prefix subtree starting at node root <root>.
 * Only new__->key and new__->pfx need be set with the key and its prefix length.
 ...
 */
static forceinline struct ebmb_node *
__ebmb_insert_prefix(struct eb_root *root, struct ebmb_node *new__, unsigned int len)
{
	struct ebmb_node *old;
	unsigned int side;
	eb_troot_t *troot, **up_ptr;
	...
	while (1) {
		...
		if (unlikely(old_node_bit < 0)) {
			...
			up_ptr = &old->node.node_p;
		check_bit_and_break:
			/* No need to compare everything if the leaves are shorter than the new__ one. */
			if (len > old->node.pfx)
				len = old->node.pfx;
			bit = equal_bits(new__->key, old->key, bit, len);
			break;
		}

		/* WARNING: for the two blocks below, <bit> is counted in half-bits */

		bit = equal_bits(new__->key, old->key, bit, old_node_bit >> 1);
		 ...
		 */
		if (bit >= (new__->node.pfx << 1) && (new__->node.pfx << 1) < old_node_bit) {
			/* insert cover node here on the left */
			new__->node.node_p = old->node.node_p;
			up_ptr = &old->node.node_p;
			new__->node.bit = new__->node.pfx << 1;
			diff = -1;
			goto insert_above;
		}
		...
		if (unlikely(bit < old_node_bit)) {
			/* The tree did not contain the key, so we insert <new__> before the
			 * node <old>, and set ->bit to designate the lowest bit position in
			 * <new__> which applies to ->branches.b[]. We know that the bit is not
			 * greater than the prefix length thanks to the test above.
			 */
			new__->node.node_p = old->node.node_p;
			up_ptr = &old->node.node_p;
			new__->node.bit = bit;
			diff = cmp_bits(new__->key, old->key, bit >> 1);
			goto insert_above;
		}
		...
		if (!(old_node_bit & 1)) {
			 ...
			 */
			if ((new__->node.pfx << 1) == old_node_bit) {
				root = &old->node.branches;
				side = EB_LEFT;
				troot = root->b[side];
				continue;
	 ...
	 *   key, and we have new__->key < old->key. We insert
	 *   new__ above old, on the left ;
	 *
	 * - the tree does not contain any leaf matching the
	 *   key, and we have new__->key > old->key. We insert
	 *   new__ above old, on the right ;
	 *
	 * - the tree does contain the key with the same prefix
	 *   length. We add the new__ key next to it as a first
	 ...
	 */
	if (bit > new__->node.pfx)
		bit = new__->node.pfx;
	if (bit > old->node.pfx)
		bit = old->node.pfx;

	new__->node.bit = (bit << 1) + 1; /* assume normal node by default */
	 ...
	 */
	diff = 0;
	if (bit < old->node.pfx && bit < new__->node.pfx)
		diff = cmp_bits(new__->key, old->key, bit);

	if (diff == 0) {
		/* Both keys match. Either it's a duplicate entry or we have to
		 * put the shortest prefix left and the largest one right below
		 * a new__ cover node. By default, diff==0 means we'll be inserted
		 * on the right.
		 */
		new__->node.bit--; /* anticipate cover node insertion */
		if (new__->node.pfx == old->node.pfx) {
			new__->node.bit = -1; /* mark as new__ dup tree, just in case */
		...
		else if (new__->node.pfx < old->node.pfx)
