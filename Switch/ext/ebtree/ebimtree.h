...
REGPRM3 struct ebpt_node *ebim_insert(struct eb_root *root, struct ebpt_node *new__, unsigned int len);
...
/* Insert ebpt_node <new__> into subtree starting at node root <root>.
 * Only new__->key needs be set with the key. The ebpt_node is returned.
 * If root->b[EB_RGHT]==1, the tree may only contain unique keys. The
 * len is specified in bytes.
 */
static forceinline struct ebpt_node *
__ebim_insert(struct eb_root *root, struct ebpt_node *new__, unsigned int len)
{
