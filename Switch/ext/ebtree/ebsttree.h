...
REGPRM2 struct ebmb_node *ebst_insert(struct eb_root *root, struct ebmb_node *new__);
...
/* Insert ebmb_node <new__> into subtree starting at node root <root>. Only
 * new__->key needs be set with the zero-terminated string key. The ebmb_node is
 * returned. If root->b[EB_RGHT]==1, the tree may only contain unique keys. The
 * caller is responsible for properly terminating the key with a zero.
 */
static forceinline struct ebmb_node *
__ebst_insert(struct eb_root *root, struct ebmb_node *new__)
{
	...
	while (1) {
		...
		side = (new__->key[old_node_bit >> 3] >> (~old_node_bit & 7)) & 1;
		troot = root->b[side];
	}
