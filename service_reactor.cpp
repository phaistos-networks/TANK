...
int Service::process_io(const int r, const uint64_t max_conn_gen) {
        for (const auto it : poller.new_events(r)) {
			...
			// because it's not the same connection as the one that was registered with epoll
                ...
                // we are checking for POLLOUT first as opposed to checking events for
                // POLLIN first, because this helps with connection::Type::Consumer connections
...
Service::flushop_res Service::flush_file_contents(connection *const c, payload *payload, const bool have_cork) {
        ...
        //for (size_t maxSpan{1}, sum{0};;)
        {
				...
				std::exchange(it.file_range.fdhandle, nullptr)->Release();
                        ...
                        if (r < outLen) {
                                ...
                                if (trace) {
                                        SLog("Socket buffer likely full\n");
                                }
...
Service::flushop_res Service::flush_iov_impl(connection *const c, struct iovec *const iov, const uint32_t iovCnt, const bool have_cork) {
        static constexpr bool trace{false};
        TANK_EXPECT(c);
        c->verify();
        ...
        if (trace) {
                ...
                for (size_t i{0}; i < iovCnt; ++i) {
                        Print(str_view32(reinterpret_cast<const char *>(iov[i].iov_base), iov[i].iov_len));
                }
        }
...
bool Service::tx(connection *const c) {
        ...
        for (auto it_ = q->front(); it_;) {
                auto *const next = it_->next;

                if (it_->src == payload::Source::DataVector) {
                        auto it = static_cast<data_vector_payload *>(it_);

                        if (0 == collected_iovs_cnt && (nullptr == next || next->src != payload::Source::DataVector)) {
                                // fast-path: just one DV to stream
                                const auto index = q->dv_iov_index;
                                const auto cnt   = it->iov_cnt;

                                if (trace) {
                                        SLog("Will send a single DV\n");
                                }
                                ...
                                switch (flush_iov_impl(c, it->iov + index, cnt - index, have_cork)) {
                                        ...
                                        case flushop_res::Flush:
                                                collected_iovs_cnt = 0;
                                                break;
                                }
                        } else {
                                const auto n = std::min<size_t>(it->iov_cnt - q->dv_iov_index,
                                                                sizeof_array(iov) - collected_iovs_cnt);

                                TANK_EXPECT(it->iov_cnt);
                                TANK_EXPECT(q->dv_iov_index < it->iov_cnt);

                                memcpy(iov + collected_iovs_cnt, it->iov + q->dv_iov_index, sizeof(struct iovec) * n);
                                collected_iovs_cnt += n;

                                if (unlikely(collected_iovs_cnt == sizeof_array(iov))) {
                                        if (false == have_cork && next) {
                                                if (trace) {
                        ...
                        if (trace) {
                                SLog("payload holds content range (collected_iovs_cnt = ", collected_iovs_cnt, ")\n");
                        }

                        if (collected_iovs_cnt) {
                                if (!have_cork) {
        ...
        if (collected_iovs_cnt) {
                if (trace) {
                        SLog("Sending pending outgoing iov[] ", collected_iovs_cnt, "\n");
                }
...
connection *Service::new_peer_connection(cluster_node *peer) {
        static constexpr bool trace{false};
