...
bool Service::close_partition_log(topic_partition *partition) {
        ...
        if (auto ptr = reinterpret_cast<void *>(const_cast<uint8_t *>(log->cur.index.ondisk.data)); ptr && ptr != MAP_FAILED) {
                munmap(ptr, log->cur.index.ondisk.span);
        }

        if (log->cur.index.fd != -1) {
                close(log->cur.index.fd);
        }
...
void Service::open_partition_log(topic_partition *partition, const partition_config &conf) {
	...
	track_accessed_partition(partition, curTime);
        ...
        if (trace) {
                SLog(ansifmt::bold, ansifmt::color_green, ansifmt::inverse, "OPENING PARTITION ", topic->name(), "/", partition->idx, ansifmt::reset, "\n");
        }
                ...
                *b.CopyTo(_base_path) = '\0';
                ...
                if (!roLogs.empty()) {
                        ...
                        for (const auto &it : roLogs) {
                                ...
                                l->roSegments->emplace_back(new ro_segment(it.firstAvailableSeqNum,
                                                                           it.lastAvailSeqNum,
                                                                           b,
                                                                           it.creationTS,
				...
				std::abort();
                        ...
                        if (const auto s = l->cur.fileSize) {
                                ...
                                for (const auto *p = data, *const e = p + span; p != e;) {
                                        ...
                                        if (!msgSetSize) {
                                                msgSetSize = Compression::decode_varuint32(p);
...
topic_partition_log *Service::partition_log(topic_partition *p) {
        TANK_EXPECT(p);
