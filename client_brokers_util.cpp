...
bool TankClient::materialize_srv_request(api_request *api_req) {
        ...
        s.counts.topics     = req_part->as_op.srv_status.counts.topics;
        s.counts.partitions = req_part->as_op.srv_status.counts.partitions;
        s.counts.nodes      = req_part->as_op.srv_status.counts.nodes;
        s.cluster_name.len  = req_part->as_op.srv_status.cluster_name.len;
        memcpy(s.cluster_name.data, req_part->as_op.srv_status.cluster_name.data, s.cluster_name.len);
...
switch_dlist *TankClient::assign_req_partitions_to_api_req(api_request *api_req, std::vector<std::pair<broker *, request_partition_ctx *>> *contexts, const uint32_t limit) {
        ...
        if (trace) {
                SLog(ansifmt::bold, ansifmt::color_blue, ansifmt::inverse, "Assigning ", contexts->size(), " to api requests (limit = ", limit, ")", ansifmt::reset, "\n");
        }
...
str_view8 TankClient::intern_topic(const str_view8 topic_name) {
        ...
        if (res.second) {
                const_cast<str_view8 *>(&res.first->first)->p = core_allocator.CopyOf(topic_name.data(), topic_name.size());
        }
