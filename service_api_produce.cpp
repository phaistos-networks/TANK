	...
	static constexpr bool trace_faults{false};
		...
		if (trace) {
			...
			const auto res = intern_map.emplace(topic_name, true);
                        ...
                        if (trace) {
                                SLog("Unknown topic [", topic_name, "] res.second = ", res.second, "\n");
                        }

                        if (res.second) {
                                // we need to retain that name
                                // so we 'll intern it here
                                auto _copy = topic_name.Copy();

                                const_cast<str_view8 *>(&res.first->first)->p = _copy;
                                pr->unknown_topic_names.emplace_back(_copy);
                        }
